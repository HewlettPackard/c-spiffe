...
static X509 ***buildChains(X509 *, map_X509_chains *, X509 **, int *,
                           x509util_CertPool *, x509util_CertPool *, err_t *);

static X509 ***considerCandidate(int type, X509 *candidate,
                                 map_X509_chains *cache, X509 **currchain,
                                 int *sigchecks, x509util_CertPool *roots,
                                 x509util_CertPool *inters, X509 ***chains,
                                 err_t *err)
{
    ...
    if(type == ROOT) {
        ...
        if(idx >= 0) {
            childchains = buildChains(candidate, cache,
                                      appendToFreshChain(currchain, candidate),
                                      sigchecks, roots, inters, err);
...
static X509 ***buildChains(X509 *cert, map_X509_chains *cache,
                           X509 **currchain, int *sigchecks,
                           x509util_CertPool *roots, x509util_CertPool *inters,
                           err_t *err)
{
    ...
    for(size_t i = 0, size = arrlenu(roots_idcs); i < size; ++i) {
        ...
        chains
            = considerCandidate(ROOT, roots->certs[root_idx], cache, currchain,
                                sigchecks, roots, inters, chains, err);
    }

    for(size_t i = 0, size = arrlenu(inters_idcs); i < size; ++i) {
        const int inter_idx = inters_idcs[i];
        chains = considerCandidate(INTERMEDIATE, inters->certs[inter_idx],
                                   cache, currchain, sigchecks, roots, inters,
                                   chains, err);
    }
...
// verify chain
static X509 ***verifyX509(X509 *cert, x509util_CertPool *roots,
                          x509util_CertPool *inters, err_t *err)
{
    *err = NO_ERROR;
    if(cert && roots) {
        X509 ***chains = NULL;
            ...
            if(!(usage & KU_KEY_CERT_SIGN) && !(usage & KU_CRL_SIGN)
               && !X509_check_ca(leaf)) {
                ...
                if(!(*err)) {
                    ...
                    X509 ***chains
                        = verifyX509(leaf, x509auths, certspool, err);
                    arrins(certs, 0, leaf);
