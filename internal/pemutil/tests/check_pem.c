...
START_TEST(test_parseBlock)
{
    ...
    const char *buffs[] = {
        ...
        "-----END RSA PRIVATE KEY-----",
        //x509 certificate (diverging types)
        "-----BEGIN CERTIFICATE-----\n"
        "MIIG4TCCBcmgAwIBAgIQCd0Ux6hVwNaX+SICZIR/jzANBgkqhkiG9w0BAQUFADBm\n"
        "MQswCQYDVQQGEwJVUzEVMBMGA1UEChMMRGlnaUNlcnQgSW5jMRkwFwYDVQQLExB3\n"
        "d3cuZGlnaWNlcnQuY29tMSUwIwYDVQQDExxEaWdpQ2VydCBIaWdoIEFzc3VyYW5j\n"
    ...
    BIO *bio_mems[] = {
        BIO_new_mem_buf((void*) buffs[0], -1),
        BIO_new_mem_buf((void*) buffs[1], -1),
        BIO_new_mem_buf((void*) buffs[2], -1),
        BIO_new_mem_buf((void*) buffs[3], -1),
        BIO_new_mem_buf((void*) buffs[4], -1),
        BIO_new_mem_buf((void*) buffs[5], -1)
    };
    ...
    {
        err_t err;
        void *data = parseBlock(bio_mems[i], str_types[i], &err);
        
        ck_assert_uint_eq(err, errs[i]);
        ck_assert_int_eq(data != NULL, not_null_ptrs[i]);
...
START_TEST(test_parseBlocks)
{
    ...
    {
        err_t err;
        void **blocks = parseBlocks((const byte*) buffs[i], type[i], &err);

        ck_assert_uint_eq(err, errs[i]);
        ck_assert_int_eq(blocks != NULL, not_null_ptrs[i]);

        if(blocks)
        {
            for(size_t j = 0, size = arrlenu(blocks); j < size; ++j)
            {
                if(!strcmp(type[i], "CERTIFICATE"))
                {
                    X509_free(blocks[j]);
                }
                else if(!strcmp(type[i], "PRIVATE KEY"))
                {
                    EVP_PKEY_free(blocks[j]);
                }
            }
            arrfree(blocks);
        }
...
Suite* pem_suite(void)
{
    ...
    tcase_add_test(tc_core, test_parseBlock);
    tcase_add_test(tc_core, test_parseBlocks);
