...
static string_t ASN1_STRING_to_string(const ASN1_STRING *asn1_str)
{
    if(asn1_str)
    {
        string_t str = string_new_range(
            (const char*) asn1_str->data,
            (const char*) asn1_str->data + asn1_str->length);
...
static string_t X509_NAME_to_string(X509_NAME *name)
{
    if(name)
    {
        ...
        string_t str = string_new_range(bio_buf->data, bio_buf->data + bio_buf->length);
        BIO_free(bio_out);
...
void x509util_CertPool_Free(x509util_CertPool *certpool)
{
    if(certpool)
    {
        ...
        for(size_t i = 0, size = shlenu(certpool->subj_keyid_idcs); i < size; ++i)
        {
