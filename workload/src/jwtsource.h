...
/** workloadapi_JWTSource is a source of JWT-SVID and JWT bundles maintained via the
 * Workload API
 * */
typedef struct {
...
/** workloadapi_NewJWTSource creates a new JWTSource. It blocks until the initial update
...
/** workloadapi_JWTSource_Close closes the source, dropping the connection to the Workload API.
...
/** workloadapi_JWTSource_WaitUntilUpdated waits until the source is updated or the context is done,
...
/** workloadapi_JWTSource_GetJWTSVID gettes a JWT-SVID from the source with the given parameters.
...
/** workloadapi_JWTSource_GetJWTBundleForTrustDomain returns the JWT bundle for the given trust
